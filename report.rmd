...
The cost of the system is a function of $S$ and $E$. We choose a simple linear function for the cost:
...
\end{tikzpicture}
\caption[Visualization of the scheduling system when the agent activates queue 2]{Visualization of the scheduling system when the agent activates queue 2, the bold lines represent the flow of jobs inside the system.}
\end{figure}
```
...
We use the same cost function of scheduling system here.

### Agent

The goal of the agent is the same: minimizing the cost of the whole system. He also receives the same information: the observable state $S$. The difference here is instead of having to deactivate some queues, all queues now run continuously. His mission is to choose which queue to send the new arriving job to. Only the chosen queue will have new jobs arriving, the other queues only have to process the remaining jobs.
...
\path ([xshift=0.5cm]q-2.south) -- ([xshift=0.5cm]q-n.north) node [font=\LARGE, midway, sloped] {$\dots$};
\end{tikzpicture}
\caption[Visualization of the load-balancing system when the agent sends jobs to queue 2]{Visualization of the load-balancing system when the agent sends job to queue 2, the bold lines represent the flow of jobs inside the system.}
\end{figure}
```
...
In this section, we document some difficulties while implementing the framework and how we overcome it. For illustration purposes, we will test the algorithm with a very simple problem of the [scheduling system](#scheduling-system)
...
```{r, 3x3-python, out.width="40%", fig.align="center", fig.cap="Result of online Q-learning using Python for 3x3 case. $\\gamma = 0.999$, $\\epsilon = 0.99$"}
```
...
```{r, 10x10-python, fig.show="hold", out.width="40%", fig.align="center", fig.cap="Result of online Q-learning using Python for 10x10 case. $\\gamma = 0.99$, $\\epsilon = 0.999$", fig.subcap=c("For $10^6$ iterations", "For $10^7$ iterations")}
```
...
```{r, 10x10-python-n-visit, fig.show="hold", out.width="40%", fig.align="center", fig.cap="Number of visits for the system above. $\\gamma = 0.99$, $\\epsilon = 0.999$", fig.subcap=c("Activating $Q_{1}$", "Activating $Q_{2}$")}
```

We can see that our agent is stuck at $(0, 0)$, only the area around the origin has a high number of visits. The rest have relatively low number of visits and therefore, do not converge because of the nonfulfillment of the condition in [@q-learning-converge].

To speed up the algorithm, we decided to switch to C++, which is famous for its speed and its mature support for high-speed computation with libraries like `Eigen` and then expose a Python interface for more ease of use. This is also the same approach for many scientific libraries in Python such as `Numpy` or  `Tensorflow`. After switching to C++ and some improvements later, the algorithm now can run $10^9$ iterations in only 1 minute, a $2 \times 10^3$ speed up.
...
The empirical value we get here will be used in \@ref(alg:value-iteration) in the case of POMDP for value iteration method.
...
First we will try with the linear cost. This is the simplest case and since it is proven mathematically in [@scheduling-optimal], it is only a test to show that all algorithms work correctly. The optimal policy here will be activating the queue whose $c_{i} \mu_{i}$ is bigger. We will try two tuples of parameters, the first one will be:
...
(ref:title-simple-scheduling-1) Result of table \@ref(tab:simple-scheduling-1) for 20x20 case. $\gamma = 0.999$, $\epsilon = 0.999$.

```{r, 20x20-simple-scheduling-1, fig.show="hold", out.width="33%", fig.align="center", fig.cap="(ref:title-simple-scheduling-1)", fig.subcap=c("Online Q-learning", "Offline Q-learning", "Value iterations")}
```
...
(ref:title-simple-scheduling-2) Result of table \@ref(tab:simple-scheduling-2) for 20x20 case. $\gamma = 0.999$, $\epsilon = 0.999$.

```{r, 20x20-simple-scheduling-2, fig.show="hold", out.width="33%", fig.align="center", fig.cap="(ref:title-simple-scheduling-2)", fig.subcap=c("Online Q-learning", "Offline Q-learning", "Value iterations")}
```
...
```{r, 20x20-simple-lb-1, fig.show="hold", out.width="33%", fig.align="center", fig.cap="(ref:title-simple-lb-1)", fig.subcap=c("Online Q-learning", "Offline Q-learning", "Value iterations")}
```

Intuitively, if two queues have the same parameters, the optimal action will be sending jobs to the queue that has fewer jobs, which is the result of the value iteration method (the right grid) of figure \@ref(fig:20x20-simple-lb-1). There is also a gray color indicating that in those states, either action $1$ (sending a new job to $Q_{1}$) or $2$ (sending a new job to $Q_{2}$) leads to the same result, because they have the same number of jobs.

For two Q-learning methods, the online and offline version has been trained for $3 \times 10^{10}$ and $5 \times 10^{7}$ iterations respectively and takes 30 minutes each. They give us a similar result and both are close to the optimal policy of value iteration.
...
(ref:title-simple-lb-2) Result of table \@ref(tab:simple-lb-2) for 20x20 case. $\gamma = 0.999$, $\epsilon = 0.999$.

```{r, 20x20-simple-lb-2, fig.show="hold", out.width="33%", fig.align="center", fig.cap="(ref:title-simple-lb-2)", fig.subcap=c("Online Q-learning", "Offline Q-learning", "Value iterations")}
```

The offline version is similar to the policy of value iteration method and has a smaller hole compared to the online version. The difference can be explained by the fact that in the offline version, each pair state-action is visited $5 \times 10^{7}$ times, so the total number of iterations will be $2 \times 41^{2} \times 5 \times 10^{7} \approx 1.6 \times 10^{11}$, which is $\gg 3 \times 10^{10}$, the total number of iterations of the online version.
...
(ref:title-simple-lb-2-n-visit) Number of visits of online Q-learning on table \@ref(tab:simple-lb-2) for 20x20 case.

```{r, 20x20-simple-lb-2-n-visit, fig.show="hold", out.width="40%", fig.align="center", fig.cap="(ref:title-simple-lb-2-n-visit)", fig.subcap=c("Sending to $Q_{1}$", "Sending to $Q_{2}$")}
```
...
(ref:title-simple-lb-3) Result of table \@ref(tab:simple-lb-3) for 20x20 case. $\gamma = 0.999$, $\epsilon = 0.999$.

```{r, 20x20-simple-lb-3, fig.show="hold", out.width="33%", fig.align="center", fig.cap="(ref:title-simple-lb-3)", fig.subcap=c("Online Q-learning", "Offline Q-learning", "Value iterations")}
```

Once again, we see the sensitivity of Q-learning with the system settings. Intuitively, since class $1$ works faster than class $2$ with the same cost, we will send more jobs to the first one as in the value iteration method. However, the learning capacity of both Q-learning methods is not sufficient to capture all the information of the system.
...
(ref:title-simple-lb-3-2-11) Result of table \@ref(tab:simple-lb-3) for 20x20 case after $2 \times 10^{11}$ iterations. $\gamma = 0.999$, $\epsilon = 0.999$.

```{r, 20x20-simple-lb-3-2-11, fig.show="hold", out.width="33%", fig.align="center", fig.cap="(ref:title-simple-lb-3-2-11)", fig.subcap=c("Online Q-learning", "Offline Q-learning", "Value iterations")}
```
...
(ref:title-simple-lb-4) Result of table \@ref(tab:simple-lb-4) for 20x20 case. $\gamma = 0.999$, $\epsilon = 0.999$.

```{r, 20x20-simple-lb-4, fig.show="hold", out.width="33%", fig.align="center", fig.cap="(ref:title-simple-lb-3)", fig.subcap=c("Online Q-learning", "Offline Q-learning", "Value iterations")}
```
...
From all the small transition matrices of each $Q_{i}$, we can get a big environment transition matrix $\mathcal{M}_{\mathcal{E}}$ where each row and column represents an environment state encoded using the method present in [From $n$-dimensions to $2$-dimensions] section. Since all the entries except the ones on the diagonal are multiplied by $\alpha$, we can express $\mathcal{M}_{\mathcal{E}}$ as follow:
...
If $\Pi_{\mathcal{E}}$ is the solution of equation \@ref(eq:steady-prob) then $\mathcal{M}_{\mathcal{E}} \Pi_{\mathcal{E}} = \Pi_{\mathcal{E}}$ (because $P(E' | E)$ is actually $\rho$).

Lastly, with all the notions above, we can prove that the solution of
equation \@ref(eq:steady-prob) does not depend on $\alpha$.
...
(ref:title-scheduling-1) Result of table \@ref(tab:scheduling-1) (upper) and \@ref(tab:scheduling-2) (lower) for 20x20 case. $\gamma = 0.999$, $\epsilon = 0.999$.

```{r, 20x20-scheduling-1-2, fig.show="hold", fig.ncol=3, out.width="33%", fig.align="center", fig.cap="(ref:title-scheduling-1)", fig.subcap=c("Online Q-learning", "Offline Q-learning", "Value iterations", "Online Q-learning", "Offline Q-learning", "Value iterations")}
```

First, for both cases, the solution of equation \@ref(eq:steady-prob) is $\Pi = (0.5, 0.5)$ with $\mathcal{E} = \{(m_{1}, m_{1}), (m_{2}, m_{1})\}$ (since queue $2$ has only one environment). Furthermore, from equation \@ref(eq:p-tilde), the value iteration method is actually a MDP problem as above with the following parameters.
...
As in [@scheduling-optimal], the solution of table \@ref(tab:mdp-scheduling-1) is serving queue $1$, which is also the result of the value iteration method on both table \@ref(tab:scheduling-1) and table \@ref(tab:scheduling-2).
...
(ref:title-scheduling-3) Result of table \@ref(tab:scheduling-3) for 20x20 case. $\gamma = 0.999$, $\epsilon = 0.999$.

```{r, 20x20-scheduling-3, fig.show="hold", out.width="33%", fig.align="center", fig.cap="(ref:title-scheduling-3)", fig.subcap=c("Online Q-learning", "Offline Q-learning", "Value iterations")}
```

The result of value iteration on the right side is also the same as the value iteration of table \@ref(tab:mdp-scheduling-3). Both actually gives the same numerical estimation of $V_{*}$. Q-learning is also good at learning this type of system and we can see that it deduces a similar policy to the value iteration's one.
...
(ref:title-scheduling-4) Result of table \@ref(tab:scheduling-4-mat-1) for 20x20 case. $\gamma = 0.999$, $\epsilon = 0.999$.

```{r, 20x20-scheduling-4, fig.show="hold", out.width="33%", fig.align="center", fig.cap="(ref:title-scheduling-4)", fig.subcap=c("Online Q-learning", "Offline Q-learning", "Value iterations")}
```

As explained above, the policy perceived by the value iteration is quite similar as above. The majority of online Q-learning cells are choosing action $1$, especially the ones near the origin. However, Q-learning still contains a lot of noise, a sign that suggests we have not iterated enough. Although the number of iterations ($3 \times 10^{10}$) is sufficiently large, bigger $\alpha$ requires a lot more iterations because in that case, the system does not evolve much in trajectory but in environment instead (for example we could think that the system will stuck at one cell near the origin and the environment state constantly changes after each iteration). The figure below shown that phenomena:

(ref:title-scheduling-4-n-visit) Number of visits of online Q-learning on table \@ref(tab:scheduling-4-mat-1) for 20x20 case. The highest number of visits is $\approx 4 \times 10^{9}$ (at $(0, 0)$) while the lowest one is $\approx 5 \times 10^5$ (at $(20, 20)$).

```{r, 20x20-scheduling-4-n-visit, fig.show="hold", out.width="40%", fig.align="center", fig.cap="(ref:title-scheduling-4-n-visit)", fig.subcap=c("Activating $Q_{1}$", "Activating $Q_{2}$")}
```

For offline Q-learning, we see no difference with the value iteration method. As in the explanation for the result of figure \@ref(fig:20x20-simple-lb-2), in the same amount of time, most of state-action pairs in offline version receives more visits than the online one (except the cells around the origin) and therefore gives a less noisy result.

We also try the complex example of table \@ref(tab:scheduling-3) with $\alpha = 10^{2}$. Below is the policy perceived by Q-learning and value iteration.

(ref:title-scheduling-5) Result of table \@ref(tab:scheduling-3) with $\alpha = 10^{2}$ for 20x20 case. $\gamma = 0.999$, $\epsilon = 0.999$.

```{r, 20x20-scheduling-5, fig.show="hold", out.width="33%", fig.align="center", fig.cap="(ref:title-scheduling-5)", fig.subcap=c("Online Q-learning", "Offline Q-learning", "Value iterations")}
```

Same as above, offline Q-learning and value iteration method give the same policy while the majority of Q-learning choose the optimal action of the empirical MDP. But it still contains some noisy cells that have not received enough visits.
...
(ref:title-lb-1) Result of table \@ref(tab:lb-1) for 20x20 case. $\gamma = 0.999$, $\epsilon = 0.999$.

```{r, 20x20-lb-1, fig.show="hold", out.width="33%", fig.align="center", fig.cap="(ref:title-lb-1)", fig.subcap=c("Online Q-learning", "Offline Q-learning", "Value iterations")}
```

The value iteration method shows the same result as expected with a neutral diagonal line. For online Q-learning, Although there is a large hole in the center of the grid, we could still see a diagonal line that split the grid into two regions as in the optimal policy of the empirical MDP. However, the offline method gives a very different result compared to the other methods.
...
(ref:title-lb-2) Result of table \@ref(tab:lb-1) with $\alpha = 10^{2}$ for 20x20 case. $\gamma = 0.999$, $\epsilon = 0.999$.

```{r, 20x20-lb-2, fig.show="hold", out.width="33%", fig.align="center", fig.cap="(ref:title-lb-1)", fig.subcap=c("Online Q-learning", "Offline Q-learning", "Value iterations")}
```

Unfortunately, the online Q-learning result here is completely random and we will need to run a much longer simulation for it to be converged. With offline version, it gives a very different, nearly opposite policy with the policy for small $\alpha$. We can see that although the offline method works well with the scheduling system, it is broken on the load-balancing system.

# Conclusion and extension

First, the table below summarizes the advantages and weaknesses of each method in our report:

| Aspect | Online Q-learning | Offline Q-learning | Value iterations |
|--|----|--|--|
| Can be used on POMDP | Yes | Yes | No |
| Prerequisites | A system simulator or nothing (by interacting directly with the real system) | A reliable system simulator | Real or calculated (empirical) parameters of the system such that it is a MDP problem |
| Checking for convergence (checking if the value does not change much after some iterations) | Since only one cell is updated at a time, and each cell is not visited uniformly, such checking requires a lot of efforts and will slow down the simulation. An alternative solution is re-running the simulation with different learning steps and comparing the result, but this also requires a lot of time and manual effort. | By calculating $\| \cdot \|_{2}$ between the previous Q-value and the current Q-value. | By calculating $\| \cdot \|_{2}$ between the previous V-value and the current V-value. |
| Speed of convergence | Since it follows the trajectory, one training session might take a very long time. In addition, because checking for convergence is not simple, we might have to run the training session multiple times as well as carefully tuning the hyper-parameters so that takes a lot of time. | Faster than online Q-learning but still taking long time before converging | The fastest method. |
| Quality of the result | The result might not converge to the best policy and contain a lot of noise. Furthermore, its result is extremely sensitive to system parameters, especially the ones having impacts on the evolution of the whole system. | The result might not converge to the best policy but it will less likely to contain noise and is more robust to a change in the settings of system. | The result converged to the optimal solution when we are working on a MDP problem. |
| Behavior on POMDP | Online Q-learning seems to perceive the problem as an empirical MDP problem as we could observe that its results resemble the value iteration result. But we need more analytical inspections to verify this hypothesis. | Same as online Q-learning | This method can not work on a POMDP problem, but said problem can be converted to an empirical MDP problem by the method describe in section [Empirical transition probability]. |

Table: Comparison between methods for MDP / POMDP problem.
...
Finally, we will end the report with an unanswered question: __"How can we know that the policy we obtained is the optimal policy in case of POMDP ?"__. This question is very important because if we have the optimal policy beforehand, not necessarily through a model free method, we can later compare it with the result of another method to get an idea if that method works with our problem or not (like what we did with the MDP problem: compare the Q-learning result and the value iteration one). However, because of the limited context of the internship, we are not able to answer that question and left it as a possibility for future work.
