...
In this section, we document the some difficulties while implementing the framework and how we overcome it. For illustration purpose, we will test the algorithm with a very simple problem of the [queuing system](#queuing-system)
...
We can see that, our agent is stuck at $(0, 0)$, only the area around the origin has a high number of visit. The rest have relatively low visit and therefore, does not converge because of the nonfulfillment of the condition in [@q-learning-converge].

To speed up the algorithm, we decided to switch to C++, which is famous for its speed and its mature support for high-speed computation with library like `Eigen` and then expose a Python interface for more ease of use. This is also the same approach for many scientific libraries in Python such as `Numpy` or  `Tensorflow`. After switching to C++ and some improvements later, the algorithm now can run $10^9$ iterations in only 1 minutes, a $2 \times 10^3$ speed up.
...
The logic behind equation \@ref(eq:p-s) and \@ref(eq:p-e) is the fact that there is only one event can happen at a time.
