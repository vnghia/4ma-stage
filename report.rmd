...
```{r, 20x20-lb-1, fig.show="hold", out.width="33%", fig.align="center", fig.cap="(ref:title-scheduling-5)", fig.subcap=c("Online Q-learning", "Offline Q-learning", "Value iterations")}
```
...
```{r, 20x20-lb-2, fig.show="hold", out.width="33%", fig.align="center", fig.cap="(ref:title-scheduling-5)", fig.subcap=c("Online Q-learning", "Offline Q-learning", "Value iterations")}
```
...
|--|----|--|--|
| Can be used on POMDP | Yes | No | No |
...
| Behavior on POMDP | Q-learning seems to perceive the problem as an empirical MDP problem as we could observe that its results resemble the value iterations result. But we need more analytical inspections to verify this hypothesis. | | This method can not work on a POMDP problem, but said problem can be converted to an empirical MDP problem by the method describe in section [Empirical transition probability]. |
