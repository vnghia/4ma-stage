...
In this section, we document some difficulties while implementing the framework and how we overcome it. For illustration purpose, we will test the algorithm with a very simple problem of the \protect\hyperlink{scheduling-system}{scheduling system}
...
\caption{Number of visits of online Q-learning on table \ref{tab:scheduling-4-mat-1} for 20x20 case. The highest number of visit is \(\approx 4 \times 10^{9}\) (at \((0, 0)\)) while the lowest one is \(\approx 5 \times 10^5\) (at \((20, 20)\)).}\label{fig:20x20-scheduling-4-n-visit}
\end{figure}
